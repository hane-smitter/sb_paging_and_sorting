...
public class EmployeeCriteriaRepository {
    ...
    public Page<Employee> findAllWithFilters(
        ...
        // 1. Predicates will filter our results in our case, with the firsName and lastName
        Predicate predicate = getPredicate(employeeSearchCriteria, employeeRoot);
        // Using our predicate
        criteriaQuery.where(predicate);
        // 2. Now to sort our results
        setOrder(employeePage, criteriaQuery, employeeRoot);

        // 3. Now we make a query to go to our database
        ...
        // Now we will create the count query to retrieve the total amount of entries
        // with applied predicates
        long employeeCount = getEmployeesCount(predicate);
    ...
    private Predicate getPredicate(
        ...
        // Our search criteria is optional(user can pass them or not)
        // So we have to first check them if they are null or not
        if (Objects.nonNull(employeeSearchCriteria.getFirstName())) {
            predicates.add(
                    criteriaBuilder.like(employeeRoot.get(
                            "firstName"), "%" + employeeSearchCriteria.getFirstName() + "%"
                    )
            );
        }
        if (Objects.nonNull(employeeSearchCriteria.getLastName())) {
            predicates.add(
                    criteriaBuilder.like(employeeRoot.get(
                            "lastName"), "%" + employeeSearchCriteria.getLastName() + "%"
                    )
            );
        }
    ...
    private void setOrder(
        ...
        if (employeePage.getSortDirection().equals(Sort.Direction.ASC)) {
            criteriaQuery.orderBy(
                    criteriaBuilder.asc((employeeRoot.get(employeePage.getSortBy())))
            );
        } else {
            criteriaQuery.orderBy(
                    criteriaBuilder.desc((employeeRoot.get(employeePage.getSortBy())))
            );
        }
    }
